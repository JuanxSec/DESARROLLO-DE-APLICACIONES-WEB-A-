-- ============================================================================
-- Esquema relacional del Proyecto Integrador JuansecCTI - version PostgreSQL
--
-- Es el mismo modelo de sql/esquema.sql, que sigue siendo el esquema de
-- referencia de la asignatura en MySQL. Esta version existe solo para el
-- despliegue en Render, que ofrece PostgreSQL gestionado. Las diferencias
-- frente al original son tres:
--
--   1. INT AUTO_INCREMENT PRIMARY KEY  ->  SERIAL PRIMARY KEY
--   2. ENUM('producto','factura')      ->  VARCHAR con restriccion CHECK
--   3. DEFAULT (CURRENT_DATE)          ->  DEFAULT CURRENT_DATE
--
-- Tampoco crea la base: en un servidor gestionado la base ya viene creada.
-- El modelo, las 15 tablas, las claves foraneas y los datos son identicos.
--
-- Ejecutar:  python init_db.py   (con DB_ENGINE=postgres)
-- ============================================================================
...
CREATE TABLE estados (
    ...
    CONSTRAINT ck_estados_ambito CHECK (ambito IN ('producto', 'factura')),
...
-- Las secuencias SERIAL no avanzan cuando el INSERT trae el id explicito, cosa
-- que aqui no ocurre, pero si se recargan datos manualmente conviene
-- sincronizarlas. Se dejan al dia de forma explicita:
SELECT setval('provincias_id_provincia_seq',       (SELECT MAX(id_provincia) FROM provincias));
SELECT setval('cantones_id_canton_seq',            (SELECT MAX(id_canton) FROM cantones));
...
SELECT setval('tipos_proveedor_id_tipo_seq',       (SELECT MAX(id_tipo) FROM tipos_proveedor));
...
SELECT setval('detalle_factura_id_detalle_seq',    (SELECT MAX(id_detalle) FROM detalle_factura));
